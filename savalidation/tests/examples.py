...
class Family(Base, ValidationMixin):
    ...
    status =  sa.Column(sa.Unicode(15), nullable=False, default='active', server_default='active')
...
class Person(Base, ValidationMixin):
    ...
    @before_flush
    def alter_name(self):
        if self.name_first == 'randy':
            self.name_first = 'randall'
    ...
    def enforce_president(self):
        ...
        if self.name_last == 'Obama' and self.name_first != 'President':
